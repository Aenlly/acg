package top.aenlly.acg.core.web.handler;
...
import top.aenlly.acg.core.service.ApiErrorLog;
...
import static top.aenlly.acg.common.enums.GlobalErrorCode.*;
...
//        log.warn("[missingServletRequestParameterExceptionHandler]", ex);
        log.warn(String.format("请求参数类型错误:%s", ex.getMessage()));
...
//        log.warn("[httpRequestMethodNotSupportedExceptionHandler]", ex);
        log.warn(String.format("请求方法不正确:%s", ex.getMessage()));
...
//        log.warn("[accessDeniedExceptionHandler][userId({}) 无法访问 url({})]", WebFrameworkUtils.getLoginUserId(req), req.getRequestURL(), ex);
        return CommonResult.error(GlobalErrorCode.FORBIDDEN);
    }
    ...
    public CommonResult<?> clientAbortExceptionHandler(HttpServletRequest req, ClientAbortException ex) {
        log.warn("[clientAbortExceptionHandler][url({})]", req.getRequestURL(), ex);
        return CommonResult.error(GlobalErrorCode.UNKNOWN);
    }
    ...
    private void initExceptionLog(ApiErrorLog errorLog, HttpServletRequest request, Throwable e) {
        ...
        errorLog.setRequestBody(ServletUtils.isJsonRequest(request) ? ServletUtil.getBody(request) : "");
